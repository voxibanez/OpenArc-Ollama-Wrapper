...
ensure_configuration_access() {
  ...
  manifest_path="$(
    ...
    source "${ENV_FILE}"
    printf '%s' "${MODEL_MANIFEST:-${CONFIG_DIR}/models.yaml}"
  )"
  [[ "${manifest_path}" == /* ]] || fail "MODEL_MANIFEST must be an absolute path; found ${manifest_path}"
  [[ -f "${manifest_path}" ]] || fail "model manifest not found: ${manifest_path}"
  chown root:"${SERVICE_GROUP}" "${manifest_path}"
  chmod 0640 "${manifest_path}"

  if ! runuser -u "${SERVICE_USER}" -- test -r "${manifest_path}"; then
    fail "${SERVICE_USER} cannot read model manifest ${manifest_path}"
  fi
  ...
  log "validated model manifest access at ${manifest_path}"
}
...
facade_ready() {
  local attempts=0
  until curl -fsS http://127.0.0.1:11434/api/version >/dev/null &&
        curl -fsS http://127.0.0.1:11434/api/tags >/dev/null; do
    attempts=$((attempts + 1))
