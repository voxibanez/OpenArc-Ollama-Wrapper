...
check_render_device() {
  ...
  RENDER_DEVICE="${RENDER_DEVICE:-${devices[0]}}"
  ...
  if [[ -z "${RENDER_GROUP}" ]]; then
    RENDER_GROUP="openarc-render"
    groupadd --gid "${RENDER_GID}" "${RENDER_GROUP}"
  fi
  log "using GPU render node ${RENDER_DEVICE} (group ${RENDER_GROUP}, gid ${RENDER_GID})"
}
...
create_service_user() {
  ...
  usermod --append --groups "${RENDER_GROUP}" "${SERVICE_USER}"

  install -d -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0750 \
    "${STATE_DIR}" "${MODEL_DIR}" "${STATE_DIR}/cache" "${STATE_DIR}/huggingface"
  install -d -o root -g "${SERVICE_USER}" -m 0750 "${CONFIG_DIR}"

  if ! runuser -u "${SERVICE_USER}" -- test -r "${RENDER_DEVICE}" ||
     ! runuser -u "${SERVICE_USER}" -- test -w "${RENDER_DEVICE}"; then
    fail "${SERVICE_USER} cannot read and write ${RENDER_DEVICE}; correct the LXC device gid/mode on the Proxmox host"
  fi
}
